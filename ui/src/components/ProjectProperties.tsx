  ...
  if (state === "saving") {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
        ...
        {i18nT("projectProperties.saving")}
      </span>
    );
  }
  if (state === "saved") {
  ...
  if (state === "error") {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] text-destructive">
        ...
        {i18nT("projectProperties.failed")}
      </span>
    );
  }
  return null;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {i18nT(`projectProperties.statusOptions.${status}`)}
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-40 p-1" align="start">
        {PROJECT_STATUSES.map((s) => (
          ...
          >
            {i18nT(`projectProperties.statusOptions.${s.value}`)}
          </Button>
        ))}
      </PopoverContent>
    </Popover>
...
function ArchiveDangerZone({
  ...
  const actionKey = isArchive ? "archiveProject" : "unarchiveProject";

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? i18nT("projectProperties.archive.archiveConfirm")
          : i18nT("projectProperties.archive.unarchiveConfirm")}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? i18nT("projectProperties.archive.archiving") : i18nT("projectProperties.archive.unarchiving")}
        </Button>
      ) : confirming ? (
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {i18nT("projectProperties.archive.confirmAction", { action: i18nT(`projectProperties.archive.${actionKey}`), name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
          ...
          >
            {i18nT("projectProperties.archive.confirm")}
          </Button>
          <Button
            size="sm"
            variant="outline"
          ...
          >
            {i18nT("projectProperties.archive.cancel")}
          </Button>
        </div>
      ) : (
        <Button
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{i18nT("projectProperties.archive.archiveProject")}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{i18nT("projectProperties.archive.unarchiveProject")}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("projectProperties.validation.pathMustBeAbsolute"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("projectProperties.validation.invalidRepoUrl"));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("projectProperties.validation.clearLocalFolder")
        : t("projectProperties.validation.deleteLocalFolder"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("projectProperties.validation.clearRepo")
        : t("projectProperties.validation.deleteRepo"),
    );
    if (!confirmed) return;
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {t("projectProperties.envDescription")}
            </p>
          </div>
        </PropertyRow>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t("projectProperties.codebase")}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {t("projectProperties.codebaseTooltip")}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t("projectProperties.paperclipManaged")}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("projectProperties.executionWorkspaces.additionalLegacyWorkspaces")}
              </div>
            )}
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? t("projectProperties.executionWorkspaces.noUrl")
                        )}
                      </div>
                    </div>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProperties.validation.failedToSave")}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProperties.validation.failedToDelete")}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProperties.validation.failedToUpdate")}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("projectProperties.executionWorkspaces.title")}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label={t("projectProperties.executionWorkspaces.title")}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {t("projectProperties.executionWorkspaces.tooltip")}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("projectProperties.executionWorkspaces.enableIsolatedCheckouts")}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("projectProperties.executionWorkspaces.enableIsolatedCheckoutsDesc")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("projectProperties.executionWorkspaces.enabled") : t("projectProperties.executionWorkspaces.disabled")}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("projectProperties.executionWorkspaces.defaultIsolated")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("projectProperties.executionWorkspaces.defaultIsolatedDesc")}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("projectProperties.executionWorkspaces.hideAdvanced")
                          : t("projectProperties.executionWorkspaces.showAdvanced")}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("projectProperties.executionWorkspaces.hostManaged")}: <span className="text-foreground">{t("projectProperties.executionWorkspaces.gitWorktree")}</span>
                        </div>
                        {environmentsEnabled ? (
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{t("projectProperties.executionWorkspaces.environment")}</span>
                            ...
                            >
                              <option value="">{t("projectProperties.executionWorkspaces.noEnvironment")}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.executionWorkspaces.baseRef")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.executionWorkspaces.branchTemplate")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.executionWorkspaces.worktreeParentDir")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.executionWorkspaces.provisionCommand")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.executionWorkspaces.teardownCommand")}</span>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("projectProperties.executionWorkspaces.provisionDescription")}
                        </p>
                      </div>
                    ) : null}
                  </div>
