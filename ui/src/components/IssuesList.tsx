        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("list.descendantProgress", { done: summary.doneCount, total: summary.totalCount })}
            </span>
            <span className="text-muted-foreground">
              {t("list.inProgress", { count: summary.inProgressCount })}
            </span>
            <span className="text-muted-foreground">
              {t("list.blocked", { count: summary.blockedCount })}
            </span>
            {showCostSummary && (
              <>
                <span
                  className="text-muted-foreground tabular-nums"
                  title={t("list.runsAcrossSubIssues", {
                    runCount: costSummary.runCount,
                    issueCount: costSummary.issueCount,
                  })}
                >
                  {formatTokens(totalTokens)} {t("list.tokens")}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {formatDurationMs(costSummary.runtimeMs)} {t("list.runtime")}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("list.subIssuesProgressAria")}
            aria-valuemin={0}
          ...
          >
            {statusEntries.map(({ status, count }) => (
              <span
                ...
                title={`${t("status:issueStatus." + status)}: ${count}`}
                aria-hidden="true"
              />
            ))}
          </div>
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next" ? t("list.nextUp") : t("list.waitingOnBlockers")}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("list.noActiveSubIssues")}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("list.allSubIssuesDone")}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("list.noActionableSubIssues")}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("list.boardUser") : t("list.me"),
        kind: "user",
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? t("list.noWorkspace") : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? t("list.noProject") : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? t("list.noParent") : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId, companyUserLabelMap) ?? t("properties.user"))
  ...
  const createActionLabel = createIssueLabel ? t("list.createLabel", { label: createIssueLabel }) : t("list.createIssue");
  const createButtonLabel = createIssueLabel ? t("list.newLabel", { label: createIssueLabel }) : t("list.newIssue");
    ...
    <div ref={rootRef} className="space-y-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? t("list.disableNesting") : t("list.enableNesting")}
            >
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("list.sort")}>
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["workflow", t("list.sortWorkflow")],
                    ["status", t("list.sortStatus")],
                    ["priority", t("list.sortPriority")],
                    ["title", t("list.sortTitle")],
                    ["created", t("list.sortCreated")],
                    ["updated", t("list.sortUpdated")],
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("list.group")}>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("list.groupStatus")],
                    ["priority", t("list.groupPriority")],
                    ["assignee", t("list.groupAssignee")],
                    ["project", t("list.groupProject")],
                    ["workspace", t("list.groupWorkspace")],
                    ["parent", t("list.groupParent")],
                    ["none", t("list.groupNone")],
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("list.searchLimitReached", { limit: ISSUE_SEARCH_RESULT_LIMIT })}
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("list.boardLimitReached", { limit: ISSUE_BOARD_COLUMN_RESULT_LIMIT })}
        </p>
      )}
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("list.noMatchingIssues")}
        ...
        <>
                ...
                const renderIssueRow = (issue: Issue, depth: number) => {
                  ...
                  const visibleBlockerChips = unresolvedVisibleBlockers
                    .map((blockerId) => {
                      ...
                      const blockerStepSuffix = blockerStep ? ` \u00b7 ${t("list.checklistStep", { number: blockerStep })}` : "";
                      return { blockerId, chipLabel: `${t("list.blockedBy", { label })}${blockerStepSuffix}` };
                    })
                  ...
                  const additionalVisibleBlockerLabel = additionalVisibleBlockerCount > 0
                    ? ` ${t("list.andMore", { count: additionalVisibleBlockerCount })}`
                    : "";
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                {t("list.subTaskCount", { count: totalDescendants })}
                              </span>
                            ) : null}
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                aria-label={t("list.needsNextStepAria")}
                                title={t("list.needsNextStep")}
                              >
                                <CircleDot className="h-3 w-3" />
                                {t("list.needsNextStep")}
                              </span>
                            ) : null}
                          </>
                        )}
                                ...
                                >
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <Identity
                                          name={assigneeUserLabel ?? t("properties.user")}
                                  ...
                                  >
                                    <input
                                      ...
                                      placeholder={t("list.searchAssignees")}
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? t("list.loadingMoreIssues")
                  : remainingIssueRowCount > 0
                    ? t("list.renderingCount", { rendered: Math.min(renderedIssueRowLimit, filtered.length), total: filtered.length })
                    : t("list.scrollToLoadMore")}
              </p>
            </div>
          )}
        </>
