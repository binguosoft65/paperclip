  ...
  return (
    <div className="flex items-start gap-1.5 min-w-0 flex-1">
      ...
      <button
        ...
        onClick={handleCopy}
        title={copied ? t("properties.copied") : t("properties.clickToCopy")}
      >
        {value}
      </button>
...
function RemovableIssueReferencePill({
  ...
  const removeLabel = t("properties.removeBlocker", { label: issueLabel });
    ...
    <>
      ...
      <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("properties.removeBlockerTitle")}</DialogTitle>
            <DialogDescription>
              {t("properties.removeBlockerDesc", { label: confirmLabel })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
              <Button type="button" variant="outline">{t("common:dialog.cancel")}</Button>
            </DialogClose>
            <Button type="button" variant="destructive" onClick={confirmRemove}>
              {t("properties.removeBlockerAction")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function IssueProperties({
  ...
  const runExecutionButton = (stageType: "review" | "approval") => (
    <PropertyRow label="">
      ...
      >
        {stageType === "review" ? t("properties.runReviewNow") : t("properties.runApprovalNow")}
      </button>
    </PropertyRow>
  );
  const currentExecutionLabel = (() => {
    if (!issue.executionState?.currentStageType) return null;
    const stageLabel = t(issue.executionState.currentStageType === "review" ? "properties.reviewStage" : "properties.approvalStage");
    ...
    if (issue.executionState.status === "changes_requested") {
      return participantLabel
        ? t("properties.changesRequestedBy", { stage: stageLabel, participant: participantLabel })
        : t("properties.changesRequested", { stage: stageLabel });
    }
    return participantLabel
      ? t("properties.pendingWith", { stage: stageLabel, participant: participantLabel })
      : t("properties.pending", { stage: stageLabel });
  })();
  useEffect(() => {
  ...
  const currentMonitorLabel = (() => {
    if (issue.executionPolicy?.monitor?.nextCheckAt) {
      return t("properties.nextCheck", { date: formatDate(new Date(issue.executionPolicy.monitor.nextCheckAt)) });
    }
    if (issue.executionState?.monitor?.status === "cleared") {
      return t("properties.cleared");
    }
    if (issue.monitorLastTriggeredAt) {
      return t("properties.lastTriggered", { time: timeAgo(issue.monitorLastTriggeredAt) });
    }
    return t("properties.notScheduled");
  })();
  ...
  const monitorTrigger = (
    <span className="inline-flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5">
      ...
      >
        {monitorNextCheckAt ? t("properties.nextCheckOffset", { offset: formatMonitorOffset(monitorNextCheckAt) }) : currentMonitorLabel}
      </span>
      {monitorNextCheckAt ? (
    ...
    <div className="flex w-full flex-col gap-2">
      <div className="flex flex-col gap-2 md:flex-row">
        ...
        <input
          ...
          placeholder={t("properties.whatToRecheck")}
      ...
      <div className="flex flex-col gap-2 md:flex-row">
        <input
          type="text"
          className="min-w-0 flex-1 rounded-md border border-border bg-transparent px-2 py-1 text-xs"
          placeholder={t("properties.externalService")}
  ...
  const labelsTrigger = selectedIssueLabels.length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("properties.noLabels")}</span>
    </>
  );
  const labelsExtra = (issue.labelIds ?? []).length > 0 ? (
    <button
      ...
      aria-label={t("properties.addLabelAria")}
      title={t("properties.addLabelAria")}
    >
  ...
  const labelsContent = (
    <>
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        ...
        >
          <Plus className="h-3 w-3" />
          {createLabel.isPending ? t("properties.creatingLabel") : t("properties.createLabel")}
        </button>
      </div>
    </>
  );
  ...
  const assigneePickerOptions = orderItemsBySelectedAndRecent(
    [
      { id: "", kind: "none" as const, label: t("properties.noAssignee"), searchText: "" },
      ...(currentUserId
        ? [{
            id: `user:${currentUserId}`,
            kind: "user" as const,
            userId: currentUserId,
            label: t("properties.assignToMe"),
      ...
      ...(issue.createdByUserId && issue.createdByUserId !== currentUserId
        ? [{
            ...
            label: creatorUserLabel ? t("properties.assignToUser", { user: creatorUserLabel }) : t("properties.assignToRequester"),
  ...
  const executionParticipantsContent = (
    ...
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={stageType === "review" ? t("properties.searchReviewers") : t("properties.searchApprovers")}
        value={search}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        <button
          ...
          onClick={onClear}
        >
          {stageType === "review" ? t("properties.noReviewers") : t("properties.noApprovers")}
        </button>
        {currentUserId && (
          <button
            className={cn(
  ...
  const projectTrigger = issue.projectId ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("properties.noProject")}</span>
    </>
  );
  const projectPickerOptions = orderItemsBySelectedAndRecent(
    [
      { id: "", kind: "none" as const, name: t("properties.noProject"), color: null as string | null },
  ...
  const parentTrigger = issue.parentId ? (
    ...
    <span className="text-sm text-muted-foreground">{t("properties.noParent")}</span>
  );
  ...
  const parentContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("properties.searchIssues")}
        value={parentSearch}
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        {inline ? (
          <div>
            <PropertyRow label={t("properties.blockedBy")}>
        ...
        <PropertyRow label={t("properties.blocking")}>
        ...
        <PropertyRow label={t("properties.subIssues")}>
        ...
        {relatedTasks.length > 0 ? (
          <PropertyRow label={t("properties.relatedTasks")}>
            <div className="flex flex-wrap gap-1">
        ...
        {currentExecutionLabel && (
          <PropertyRow label={t("properties.execution")}>
        ...
        <>
          ...
          <div className="space-y-1">
            {liveWorkspaceService?.url && (
              <PropertyRow label={t("properties.service")}>
                <a
            ...
            {showWorkspaceDetailLink && issue.executionWorkspaceId && (
              <PropertyRow label={t("properties.workspace")}>
                ...
                >
                  {t("properties.viewWorkspace")}
                  <ExternalLink className="h-3 w-3" />
                </Link>
              </PropertyRow>
            )}
            {workspaceTasksExecutionWorkspaceId && (
              <PropertyRow label={t("properties.tasks")}>
                <Link
                  to={executionWorkspaceIssuesHref(workspaceTasksExecutionWorkspaceId)}
                  className="text-sm text-primary hover:underline inline-flex items-center gap-1"
                >
                  {t("properties.viewWorkspaceTasks")}
                  <ExternalLink className="h-3 w-3" />
                </Link>
              </PropertyRow>
            )}
            {issue.currentExecutionWorkspace?.branchName && (
              <PropertyRow label={t("properties.branch")}>
                <TruncatedCopyable
            ...
            {issue.currentExecutionWorkspace?.cwd && (
              <PropertyRow label={t("properties.folder")}>
                <TruncatedCopyable
