...
export function NewProjectDialog() {
  ...
  const deriveWorkspaceNameFromPath = (value: string) => {
    ...
    return segments[segments.length - 1] ?? t("newProjectDialog.derivedNames.localFolder");
  };
  ...
  const deriveWorkspaceNameFromRepo = (value: string) => {
    try {
      ...
      return repo || t("newProjectDialog.derivedNames.gitHubRepo");
    } catch {
      return t("newProjectDialog.derivedNames.gitHubRepo");
    }
  };
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("newProjectDialog.validation.pathMustBeAbsolute"));
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("newProjectDialog.validation.invalidRepoUrl"));
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("newProjectDialog.title")}</span>
          </div>
            ...
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <Tooltip delayDuration={300}>
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProjectDialog.tooltips.repoUrl")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
              ...
              placeholder={t("placeholder.repoUrl")}
            />
          </div>
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <Tooltip delayDuration={300}>
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProjectDialog.tooltips.localFolder")}
                </TooltipContent>
              </Tooltip>
            </div>
          ...
          <Popover open={statusOpen} onOpenChange={setStatusOpen}>
            ...
            <PopoverContent className="w-40 p-1" align="start">
              {projectStatuses.map((s) => (
                ...
                >
                  {t(`newProjectDialog.statusOptions.${s.value}`)}
                </button>
              ))}
            </PopoverContent>
          </Popover>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("newProjectDialog.failedToCreate")}</p>
          ) : (
            <span />
          )}
          <Button
