...
function issueColumnLabels(t: (key: string) => string): Record<InboxIssueColumn, string> {
  return {
    ...
    parent: t("parentIssue"),
    labels: t("tags"),
    updated: t("lastUpdated"),
  };
}

function issueColumnDescriptions(t: (key: string) => string): Record<InboxIssueColumn, string> {
  return {
    status: t("descStatus"),
    id: t("descId"),
    assignee: t("descAssignee"),
    project: t("descProject"),
    workspace: t("descWorkspace"),
    parent: t("descParent"),
    labels: t("descLabels"),
    updated: t("descUpdated"),
  };
}
...
export function IssueColumnPicker({
  ...
  const labels = issueColumnLabels(t);
  const descriptions = issueColumnDescriptions(t);
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
        ...
        >
          ...
          <span className="ml-auto text-xs text-muted-foreground">{t("columns.defaultHint")}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel = assigneeUserName ?? formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("columns.unassigned");
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("columns.subIssue")}</span>
              )}
            </span>
          );
        }
