...
export function NewAgent() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("newAgent.breadcrumbAgents"), href: "/agents" },
      { label: t("newAgent.breadcrumbNewAgent") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("newAgent.failedToCreate"));
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      if (!isValidOpenCodeModelId(configValues.model)) {
        setFormError(t("newAgent.openCodeModelError"));
        return;
      }
    }
    createAgent.mutate(
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-lg font-semibold">{t("newAgent.heading")}</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {t("newAgent.description")}
        </p>
      </div>
      ...
      <div className="border border-border">
        ...
        <div className="border-t border-border px-4 py-4">
          <div className="space-y-3">
            <div>
              <h2 className="text-sm font-medium">{t("newAgent.companySkills")}</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("newAgent.companySkillsDesc")}
              </p>
            </div>
            {availableSkills.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {t("newAgent.noCompanySkills")}
              </p>
            ) : (
              <div className="space-y-3">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {testAgentState.pending ? t("newAgent.testing") : t("newAgent.testAgent")}
                </Button>
                <Button
                  size="sm"
