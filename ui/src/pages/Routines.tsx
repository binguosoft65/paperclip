...
export function buildRoutineGroups(
  ...
  if (groupByValue === "project") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        const noProject = t?.("routines.noProject") ?? "No project";
        const unknownProject = t?.("routines.unknownProject") ?? "Unknown project";
      ...
      .map((key) => ({
        key,
        label: key === "__no_project" ? (t?.("routines.noProject") ?? "No project") : (projectById.get(key)?.name ?? t?.("routines.unknownProject") ?? "Unknown project"),
        items: groups[key]!,
      }));
  }
  ...
  return Object.keys(groups)
    .sort((left, right) => {
      const unassigned = t?.("routines.unassigned") ?? "Unassigned";
      const unknownAgent = t?.("routines.unknownAgent") ?? "Unknown agent";
    ...
    .map((key) => ({
      key,
      label: key === "__unassigned" ? (t?.("routines.unassigned") ?? "Unassigned") : (agentById.get(key)?.name ?? t?.("routines.unknownAgent") ?? "Unknown agent"),
      items: groups[key]!,
    }));
}
...
export function Routines() {
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.toast.created"),
        body: routine.assigneeAgentId
          ? t("routines.toast.addTrigger")
          : t("routines.toast.draftSaved"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast.failedUpdate"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.updateError"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast.runFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.runError"),
        tone: "error",
      });
    },
  });
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        ...
        body: t("routines.toast.setDefaultAgent"),
        tone: "warn",
      });
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines.tabRoutines") },
            { value: "runs", label: t("routines.tabRecentRuns") },
          ]}
        />
            ...
            <div className="flex items-center gap-1">
              <Popover>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("routines.sortUpdated")],
                      ["created", t("routines.sortCreated")],
                      ["lastRun", t("routines.sortLastRun")],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? t("routines.sortAsc") : t("routines.sortDesc")}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["project", t("routines.groupProject")],
                      ["assignee", t("routines.groupAgent")],
                      ["none", t("routines.groupNone")],
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">{t(`routines.concurrencyDescription.${draft.concurrencyPolicy}`)}</p>
                    </div>
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">{t(`routines.catchUpDescription.${draft.catchUpPolicy}`)}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
