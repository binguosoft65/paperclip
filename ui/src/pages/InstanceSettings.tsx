...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceSettings.breadcrumbSettings") },
      { label: t("instanceSettings.breadcrumbHeartbeats") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.failedToUpdate"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.failedDisableSingle", { detail })
            : t("instanceSettings.failedDisableMultiple", { count: failures.length, enabled: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.failedDisableAll"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.loading")}</div>;
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("instanceSettings.description")}
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("instanceSettings.company") : t("instanceSettings.companies")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("instanceSettings.confirmDisableNoun_one") : t("instanceSettings.confirmDisableNoun_other");
              if (!window.confirm(t("instanceSettings.confirmDisable", { count: enabledCount, noun }))) {
                return;
              }
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.noHeartbeats")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("instanceSettings.heartbeatOn") : t("instanceSettings.heartbeatOff")}
                        </Badge>
                        <Link
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("instanceSettings.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("instanceSettings.disableTimerHeartbeat") : t("instanceSettings.enableTimerHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
