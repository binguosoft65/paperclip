...
export function CompanyAccess() {
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("access.toastMemberUpdated"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("access.toastFailedUpdateMember"),
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("access.toastJoinApproved"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("access.toastFailedApproveJoin"),
        body: error instanceof Error ? error.message : t("access.toastUnknownError"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("access.toastJoinRejected"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("access.toastFailedRejectJoin"),
        body: error instanceof Error ? error.message : t("access.toastUnknownError"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("access.toastMemberRemoved"),
        body:
          result.reassignedIssueCount > 0
            ? t("access.toastIssuesCleanedUp", { count: result.reassignedIssueCount, plural: result.reassignedIssueCount === 1 ? t("access.toastIssueSingular") : t("access.toastIssuePlural") })
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("access.toastFailedRemoveMember"),
        body: error instanceof Error ? error.message : t("access.toastUnknownError"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("access.noCompanySelected")}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("access.loading")}</div>;
  }
    ...
    <div className="max-w-6xl space-y-8">
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("access.humansTitle")}</h2>
          </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("access.humansDescription")}
          </p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("access.pendingJoinsTitle")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("access.pendingJoinsDescription")}
                </p>
              </div>
              <Badge variant="outline">{t("access.pendingBadge", { count: pendingHumanJoinRequests.length })}</Badge>
            </div>
            <div className="space-y-3">
                ...
                <PendingJoinRequestCard
                  ...
                  context={
                    request.invite
                      ...
                      : t("access.inviteMetaUnavailable")
                  }
                  detail={`Submitted ${new Date(request.createdAt).toLocaleString()}`}
                  approveLabel={t("access.approveHuman")}
                  rejectLabel={t("access.rejectHuman")}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-[minmax(0,1.5fr)_120px_120px_minmax(0,1.2fr)_180px] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{t("access.columns.userAccount")}</div>
            <div>{t("access.columns.role")}</div>
            <div>{t("access.columns.status")}</div>
            <div>{t("access.columns.grants")}</div>
            <div className="text-right">{t("access.columns.action")}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("access.noMembers")}</div>
          ) : (
            members.map((member) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t("access.editMemberDialog.title")}</DialogTitle>
            <DialogDescription>
              {t("access.editMemberDialog.description", { name: editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId })}
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("access.editMemberDialog.companyRole")}</span>
                  <select
                  ...
                  >
                    <option value="">{t("access.role.unset")}</option>
                ...
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("access.editMemberDialog.membershipStatus")}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
                  ...
                  >
                    <option value="active">{t("access.membershipStatus.active")}</option>
                    <option value="pending">{t("access.membershipStatus.pending")}</option>
                    <option value="suspended">{t("access.membershipStatus.suspended")}</option>
                  </select>
                </label>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-sm font-medium">{t("access.editMemberDialog.grantsTitle")}</h3>
                  <p className="text-sm text-muted-foreground">
                    {t("access.editMemberDialog.grantsDescription")}
                  </p>
                </div>
                <div className="rounded-lg border border-border px-3 py-3">
                  <div className="text-sm font-medium">{t("access.editMemberDialog.implicitGrantsTitle")}</div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {draftRole
                      ? t("access.editMemberDialog.implicitGrantsFromRole", { role: HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] })
                      : t("access.editMemberDialog.implicitGrantsDescription")}
                  </p>
                      ...
                      <span className="space-y-1">
                        ...
                        {implicitGrantSet.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("access.editMemberDialog.implicitGrantIncluded", { role: draftRole ? HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] : t("access.membershipStatus.active") })}
                          </span>
                        ) : null}
                        {draftGrants.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("access.editMemberDialog.explicitGrantStored")}
                          </span>
                        ) : null}
                      </span>
                    </label>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingMemberId(null)}>
              {t("access.editMemberDialog.cancel")}
            </Button>
            <Button
              onClick={() => {
                if (!editingMember) return;
            ...
            >
              {updateMemberMutation.isPending ? t("access.editMemberDialog.saving") : t("access.editMemberDialog.saveAccess")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("access.removeMemberDialog.title")}</DialogTitle>
            <DialogDescription>
              {t("access.removeMemberDialog.description", { name: memberDisplayName(removingMember) })}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("access.removeMemberDialog.loadingAssigned")
                    : t("access.removeMemberDialog.assignedIssuesCount", { count: assignedIssues.length, plural: assignedIssues.length === 1 ? t("access.removeMemberDialog.issueSingular") : t("access.removeMemberDialog.issuePlural") })}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("access.removeMemberDialog.issueReassignment")}</div>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                    value={reassignmentTarget}
                    onChange={(event) => setReassignmentTarget(event.target.value)}
                  >
                    <option value="__unassigned">{t("access.removeMemberDialog.leaveUnassigned")}</option>
                    {activeReassignmentUsers.length > 0 ? (
                      <optgroup label={t("access.removeMemberDialog.humansGroup")}>
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={t("access.removeMemberDialog.agentsGroup")}>
                  ...
                  <div className="max-h-36 overflow-auto rounded-lg border border-border">
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {t("access.removeMemberDialog.moreIssues", { count: assignedIssues.length - 6, plural: assignedIssues.length - 6 === 1 ? t("access.removeMemberDialog.moreIssueSingular") : t("access.removeMemberDialog.moreIssuePlural") })}
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRemovingMemberId(null)}>
              {t("access.removeMemberDialog.cancel")}
            </Button>
            <Button
              variant="destructive"
              onClick={() => {
            ...
            >
              {archiveMemberMutation.isPending ? t("access.removeMemberDialog.removing") : t("access.removeMemberDialog.removeButton")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
