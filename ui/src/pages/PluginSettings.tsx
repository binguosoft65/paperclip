 ...
 * @see docs/plugins/PLUGIN_SPEC.md §13 — Plugin Health Checks.
 * @see docs/plugins/PLUGIN_SPEC.md §19.8 — Plugin Settings UI.
 */
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("app.paperclip"), href: "/dashboard" },
      { label: t("pluginSettings.breadcrumbSettings"), href: "/instance/settings/heartbeats" },
      { label: t("pluginSettings.breadcrumbPlugins"), href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? plugin?.id ?? t("pluginSettings.breadcrumbDetail") },
    ]);
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("pluginSettings.noDescription");
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("pluginSettings.tabConfiguration") },
            { value: "status", label: t("pluginSettings.tabStatus") },
          ]}
          value={activeTab}
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("pluginSettings.aboutTitle")}</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                ...
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("pluginSettings.authorLabel")}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t("pluginSettings.categoriesLabel")}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("pluginSettings.categoriesNone")}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("pluginSettings.settingsTitle")}</h2>
              </div>
              {hasLocalFolders ? (
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("pluginSettings.configFromEnvironments")}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t("pluginSettings.configFromEnvironmentsDesc", { driverLabel: driverLabel || t("pluginSettings.driverLabel") })}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/environments">
                      <Button variant="outline" size="sm">{t("pluginSettings.openCompanyEnvironments")}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
        ...
        <TabsContent value="status" className="space-y-6">
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_320px]">
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("pluginSettings.runtimeDashboardDesc")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                        ...
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pluginSettings.workerStatus")}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pluginSettings.workerPid")}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pluginSettings.workerUptime")}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pluginSettings.workerPendingRpcs")}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("pluginSettings.workerCrashes")}
                                  </span>
                                  <span className="text-xs">
                                    {t("pluginSettings.workerCrashesDetail", { consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("pluginSettings.workerLastCrash")}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pluginSettings.noWorker")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pluginSettings.noJobRuns")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pluginSettings.noWebhookDeliveries")}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("pluginSettings.lastChecked", { time: new Date(dashboardData.checkedAt).toLocaleTimeString() })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("pluginSettings.runtimeUnavailable")}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("pluginSettings.recentLogsDesc", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
                  ...
                  {healthLoading ? (
                    ...
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("pluginSettings.healthOverall")}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("pluginSettings.healthLifecycle")}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("pluginSettings.healthChecksDesc")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("pluginSettings.detailsTitle")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("pluginSettings.detailsPluginId")}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("pluginSettings.detailsPluginKey")}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("pluginSettings.detailsNpmPackage")}</span>
                    <span className="max-w-[170px] truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("pluginSettings.detailsVersion")}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("pluginSettings.permissionsTitle")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("pluginSettings.noPermissions")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("pluginSettings.localFolder.selectCompany")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("pluginSettings.localFolder.title")}</h3>
      </div>
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("pluginSettings.localFolder.loadFailed")}
        </div>
      ) : null}
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          {t("pluginSettings.localFolder.loading")}
        </div>
      ) : (
        <div className="space-y-3">
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("pluginSettings.localFolder.saved")
          : t("pluginSettings.localFolder.savedWithIssues"),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("pluginSettings.localFolder.saveFailed") });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t("pluginSettings.localFolder.pathRequired") });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("pluginSettings.localFolder.pathAbsolute") });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? t("pluginSettings.localFolder.healthy") : t("pluginSettings.localFolder.needsAttention")}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? t("pluginSettings.localFolder.readWrite") : t("pluginSettings.localFolder.readOnly")}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={t("pluginSettings.localFolder.configured")} value={status?.configured ? t("pluginSettings.localFolder.yes") : t("pluginSettings.localFolder.no")} ok={!!status?.configured} />
        <FolderStatusMetric label={t("pluginSettings.localFolder.readable")} value={status?.readable ? t("pluginSettings.localFolder.yes") : t("pluginSettings.localFolder.no")} ok={!!status?.readable} />
        <FolderStatusMetric
          label={t("pluginSettings.localFolder.writable")}
          value={access === "read" ? t("pluginSettings.localFolder.notRequested") : status?.writable ? t("pluginSettings.localFolder.yes") : t("pluginSettings.localFolder.no")}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t("pluginSettings.localFolder.configuredPath")}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t("pluginSettings.localFolder.pathLabel")}
        </label>
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={t("pluginSettings.localFolder.pathPlaceholder")}
          />
          ...
          >
            ...
            {t("pluginSettings.localFolder.save")}
          </Button>
        </div>
      </div>
      ...
      {status?.problems?.length ? (
        <div className="space-y-2 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <div className="font-medium">{t("pluginSettings.localFolder.validationProblems")}</div>
...
function FolderRequirements({
  ...
  return (
    <div className="grid gap-3 text-sm md:grid-cols-2">
      <RequirementList
        title={t("pluginSettings.localFolder.requiredDirectories")}
        items={requiredDirectories}
        missingItems={missingDirectories}
        missingLabel={t("pluginSettings.localFolder.missingDirectories")}
        inspectionUnavailable={rootNotInspected}
      />
      <RequirementList
        title={t("pluginSettings.localFolder.requiredFiles")}
        items={requiredFiles}
        missingItems={missingFiles}
        missingLabel={t("pluginSettings.localFolder.missingFiles")}
        inspectionUnavailable={rootNotInspected}
      />
    </div>
  );
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          <Badge variant="secondary" className="text-[10px]">
            {t("pluginSettings.localFolder.notInspected")}
          </Badge>
        ) : missingItems.length > 0 ? (
          <Badge variant="destructive" className="text-[10px]">
            {t("pluginSettings.localFolder.missingCount", { count: missingItems.length })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-[10px]">{t("pluginSettings.localFolder.present")}</Badge>
        )}
      </div>
      {items.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("pluginSettings.localFolder.noneDeclared")}</p>
      )}
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t("pluginSettings.localFolder.rootNotInspected")}</p>
      ) : missingItems.length > 0 ? (
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("pluginSettings.configSaved") });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("pluginSettings.configSaveFailed") });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("pluginSettings.configTestPassed") });
      } else {
        setTestResult({ type: "error", text: result.message || t("pluginSettings.configTestFailed") });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("pluginSettings.configTestFailed") });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        <Loader2 className="h-4 w-4 animate-spin" />
        {t("pluginSettings.loadingConfig")}
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("pluginSettings.configSaving")}
            </>
          ) : (
            t("pluginSettings.configSave")
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("pluginSettings.configTesting")}
              </>
            ) : (
              t("pluginSettings.configTest")
            )}
          </Button>
        )}
      </div>
