...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    <span className="inline-flex items-center gap-1 text-green-700 dark:text-green-400">
      ...
      {t("environments.support.yes")}
    </span>
  ) : (
    <span className="text-muted-foreground">{t("environments.support.no")}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("breadcrumb.environments") },
    ]);
  ...
  const environmentMutation = useMutation({
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId ? t("environments.toastUpdated") : t("environments.toastCreated"),
        body: t("environments.toastReady", { name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("environments.toastFailedSave"),
        body: error instanceof Error ? error.message : t("environments.toastSaveFailed"),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok ? t("environments.toastProbePassed") : t("environments.toastProbeFailed"),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      pushToast({
        title: t("environments.toastProbeFailed"),
        body: error instanceof Error ? error.message : t("environments.toastProbeFailed"),
        tone: "error",
      });
    },
  });
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok ? t("environments.toastDraftProbePassed") : t("environments.toastDraftProbeFailed"),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("environments.toastDraftProbeFailed"),
        body: error instanceof Error ? error.message : t("environments.toastProbeFailed"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("environments.noCompanySelected")}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("environments.notEnabledTitle")}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("environments.notEnabledDescription")}
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      ...
      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("environments.adapterSupportNote")}
        </div>

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("environments.table.caption")}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("environments.table.adapter")}</th>
                <th className="px-3 py-2 font-medium">{t("environments.table.local")}</th>
                <th className="px-3 py-2 font-medium">{t("environments.table.ssh")}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("environments.table.sandbox")}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("environments.empty")}</div>
          ) : (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {environment.driver === "ssh" ? (
                        <div className="text-xs text-muted-foreground">
                          {typeof environment.config.host === "string" ? environment.config.host : t("environments.fieldHost")} ·{" "}
                          {typeof environment.config.username === "string" ? environment.config.username : t("environments.fieldUsername")}
                        </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {environment.driver !== "local" ? (
                        ...
                        >
                          {environmentProbeMutation.isPending
                            ? t("environments.testing")
                            : environment.driver === "ssh"
                              ? t("environments.testConnectionButton")
                              : t("environments.testProviderButton")}
                        </Button>
                      ) : null}
                      <Button
                        size="sm"
                      ...
                      >
                        {isEditing ? t("environments.editingLabel") : t("environments.editLabel")}
                      </Button>
                    </div>
                  </div>
                  {probe ? (
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId ? t("environments.editFormTitle") : t("environments.addFormTitle")}
          </div>
          <div className="space-y-3">
            <Field label={t("environments.fieldName")} hint={t("environments.fieldNameHint")}>
              <input
            ...
            <Field label={t("environments.fieldDescription")} hint={t("environments.fieldDescriptionHint")}>
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field label={t("environments.fieldDriver")} hint={t("environments.fieldDriverHint")}>
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              >
                <option value="ssh">{t("environments.optionSsh")}</option>
                {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                  <option value="sandbox">{t("environments.optionSandbox")}</option>
                ) : null}
                <option value="local">{t("environments.optionLocal")}</option>
              </select>
            </Field>

            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("environments.fieldHost")} hint={t("environments.fieldHostHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("environments.fieldPort")} hint={t("environments.fieldPortHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="number"
                    min={1}
                ...
                <Field label={t("environments.fieldUsername")} hint={t("environments.fieldUsernameHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("environments.fieldRemoteWorkspacePath")} hint={t("environments.fieldRemoteWorkspacePathHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                    placeholder={t("environments.fieldRemoteWorkspacePathPlaceholder")}
                    value={environmentForm.sshRemoteWorkspacePath}
                    onChange={(e) =>
                      setEnvironmentForm((current) => ({ ...current, sshRemoteWorkspacePath: e.target.value }))}
                  />
                </Field>
                <Field label={t("environments.fieldPrivateKey")} hint={t("environments.fieldPrivateKeyHint")}>
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    ...
                    >
                      <option value="">{t("environments.fieldNoSavedSecret")}</option>
                      {(secrets ?? []).map((secret) => (
                ...
                <Field label={t("environments.fieldKnownHosts")} hint={t("environments.fieldKnownHostsHint")}>
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("environments.fieldStrictHostKeyChecking")}
                    hint={t("environments.fieldStrictHostKeyCheckingHint")}
            ...
            {environmentForm.driver === "sandbox" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("environments.fieldProvider")} hint={t("environments.fieldProviderHint")}>
                  <select
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                      {t("environments.fieldNoConfigFields")}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {environmentMutation.isPending
                  ? editingEnvironmentId
                    ? t("environments.saving")
                    : t("environments.creating")
                  : editingEnvironmentId
              ...
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending ? t("environments.testingDraft") : t("environments.testDraftButton")}
                </Button>
              ) : null}
