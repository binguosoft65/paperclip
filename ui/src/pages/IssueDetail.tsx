...
function treeControlLabel(t: (key: string) => string, mode: IssueTreeControlMode, scope: "leaf" | "subtree") {
  if (scope === "leaf") {
    const leafLabels: Partial<Record<IssueTreeControlMode, string>> = {
      pause: t("detail.pauseWork"),
      resume: t("detail.resumeWork"),
    };
    return leafLabels[mode] ?? t("detail." + mode + "Subtree");
  }
  return t("detail." + mode + "Subtree");
}

function treeControlHelpText(t: (key: string) => string, mode: IssueTreeControlMode, scope: "leaf" | "subtree") {
  if (scope === "leaf") {
    return t("detail." + mode + "LeafHelp");
  }
  return t("detail." + mode + "SubtreeHelp");
}

function treeControlPreviewErrorCopy(t: (key: string) => string, error: unknown): string {
  if (error instanceof ApiError) {
    if (error.status === 403) return t("detail.previewForbidden");
    if (error.status === 409) return t("detail.previewStale");
    if (error.status === 422) return t("detail.previewInvalid");
  }
  return error instanceof Error ? error.message : t("detail.unableToLoadPreview");
}
...
function ActorIdentity({ evt, agentMap, userProfileMap }: { evt: ActivityEvent; agentMap: Map<string, Agent>; userProfileMap?: Map<string, import("../lib/company-members").CompanyUserProfile> }) {
  ...
  if (evt.actorType === "system") return <Identity name={t("detail.systemUser")} size="sm" />;
  if (evt.actorType === "user") {
    const profile = userProfileMap?.get(id);
    return <Identity name={profile?.label ?? t("detail.boardUser")} avatarUrl={profile?.image} size="sm" />;
  }
  return <Identity name={id || t("detail.unknownUser")} size="sm" />;
}
      ...
      <div className="ml-auto flex items-center gap-0.5">
        {issueIdProp && !issueHidden && (
          <Button
            ...
            aria-label={t("detail.archiveFromInbox")}
          >
        ...
        <Popover open={menuOpen} onOpenChange={setMenuOpen}>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("detail.moreIssueActions")}>
    ...
    <div className="space-y-3">
      {hasOlderComments ? (
        <div className="flex justify-center">
          ...
          >
            {commentsLoadingOlder ? t("detail.loadingEarlierComments") : t("detail.loadEarlierComments")}
          </Button>
        </div>
      ) : null}
      <IssueChatThread
        ...
        stopRunLabel={t("detail.pauseWork")}
    ...
    <>
      {shouldShowCostSummary && (
        <div className="mb-3 px-3 py-2 rounded-lg border border-border">
          <div className="text-sm font-medium text-muted-foreground mb-1">{t("detail.costSummary")}</div>
          {!issueCostSummary.hasCost && !issueCostSummary.hasTokens && !hasIssueTreeCost ? (
            <div className="text-xs text-muted-foreground">{t("detail.noCostData")}</div>
          ) : (
            <div className="space-y-1 text-xs text-muted-foreground tabular-nums">
              <div className="flex flex-wrap gap-3">
                <span className="font-medium text-foreground">{t("detail.thisIssue")}</span>
...
export function IssueDetail() {
  ...
  const breadcrumbTitle = issue?.title ?? issueId ?? t("detail.issueLabel");
  ...
  const updateIssue = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("detail.issueUpdateFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToSaveIssueChanges"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const executeTreeControl = useMutation({
    ...
    onSuccess: async (result) => {
      const modeLabel = treeControlLabel(t, result.hold.mode, treeControlScope);
      const cancelCount = result.preview?.totals.activeRuns ?? 0;
      pushToast({
        title: result.kind === "release"
          ? treeControlScope === "leaf" ? t("detail.workResumed") : t("detail.subtreeResumed")
          : result.hold.mode === "pause"
            ? treeControlScope === "leaf" ? t("detail.workPaused") : t("detail.subtreePaused")
            : t("detail.controlApplied", { mode: modeLabel }),
        body: result.kind === "release"
          ? (result.hold.releaseReason?.trim() || (treeControlScope === "leaf" ? t("detail.leafPauseReleased") : t("detail.subtreePauseReleased")))
          : result.hold.mode === "pause"
            ? treeControlScope === "leaf"
              ? t("detail.workPausedCancelled", { count: cancelCount })
              : t("detail.subtreePausedCancelled", { count: cancelCount })
            : result.hold.reason?.trim()
              ? result.hold.reason
              : t("detail.subtreePaused"),
      });
    ...
    onError: (err) => {
      pushToast({
        title: t("detail.unableToApplyControl"),
        body: err instanceof Error ? err.message : t("detail.pleaseTryAgain"),
        tone: "error",
      });
    },
  });
  const pauseIssueWorkRun = useMutation({
    ...
    onSuccess: async (result) => {
      const cancelCount = result.preview?.totals.activeRuns ?? 0;
      pushToast({
        title: t("detail.workPaused"),
        body: cancelCount > 0
          ? t("detail.workPausedCancelled", { count: cancelCount })
          : t("detail.workPausedHeld"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.issues.detail(issueId!) }),
    ...
    onError: (err) => {
      pushToast({
        title: t("detail.unableToPauseWork"),
        body: err instanceof Error ? err.message : t("detail.pleaseTryAgain"),
        tone: "error",
      });
    },
  ...
  const updateChildIssue = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("detail.issueUpdateFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToSaveSubIssueChanges"),
        tone: "error",
      });
    },
  });
  ...
  const checkIssueMonitorNow = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("detail.monitorCheckQueued"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("detail.monitorCheckFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToTriggerMonitor"),
        tone: "error",
      });
    },
  });
  ...
  const approvalDecision = useMutation({
    ...
    onSuccess: (_approval, variables) => {
      ...
      pushToast({
        title: variables.action === "approve" ? t("detail.approvalApproved") : t("detail.approvalRejected"),
        tone: "success",
      });
    },
    onError: (err, variables) => {
      pushToast({
        title: variables.action === "approve" ? t("detail.approvalFailed") : t("detail.rejectionFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToUpdateApproval"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const addComment = useMutation({
    ...
    onSuccess: async (comment, _variables, context) => {
      ...
      if (context?.optimisticCommentId && cancelledQueuedOptimisticCommentIdsRef.current.has(context.optimisticCommentId)) {
        ...
        try {
          ...
          pushToast({
            title: t("detail.cancelFailed"),
            body: err instanceof Error ? err.message : t("detail.unableToCancelComment"),
            tone: "error",
          });
        }
      }
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("detail.unableToPostComment"),
        tone: "error",
      });
    },
    ...
    onSuccess: (interaction) => {
      ...
      pushToast({
        title: interaction.kind === "request_confirmation"
          ? t("detail.requestConfirmed")
          : skippedCount > 0
          ? t("detail.acceptedDraftsSkipped", { createdCount, skippedCount })
          : t("detail.suggestedTasksAccepted"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("detail.acceptFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToAcceptTasks"),
        tone: "error",
      });
    },
  });
  const rejectInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: interaction.kind === "request_confirmation" ? t("detail.requestDeclined") : t("detail.suggestionRejected"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("detail.rejectFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToRejectTasks"),
        tone: "error",
      });
    },
  });
  const answerInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("detail.answersSubmitted"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("detail.submitFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToSubmitAnswers"),
        tone: "error",
      });
    },
  });
  ...
  const cancelInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("detail.questionCancelled"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("detail.cancelFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToCancelQuestion"),
        tone: "error",
      });
    },
  });
  ...
  const interruptQueuedComment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueRunState();
      pushToast({
        title: t("detail.interruptRequested"),
        body: t("detail.interruptBody"),
        tone: "success",
      });
    },
    onError: (err, _runId, context) => {
      ...
      pushToast({
        title: t("detail.interruptFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToInterruptRun"),
        tone: "error",
      });
    },
  });
  ...
  const cancelQueuedComment = useMutation({
    ...
    onSuccess: (comment) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("detail.queuedCommentCanceled"),
        body: t("detail.queuedCommentRestoredBody"),
        tone: "success",
      });
    },
    onError: (err) => {
  ...
  const feedbackVoteMutation = useMutation({
        ...
        title:
          variables.sharingPreferenceAtSubmit === "prompt"
            ? variables.allowSharing
              ? t("detail.feedbackSavedFutureShare")
              : t("detail.feedbackSavedFutureLocal")
            : variables.allowSharing
              ? t("detail.feedbackSavedAndShared")
              : "Feedback saved",
        tone: "success",
      });
    },
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("detail.failedToSaveFeedback"),
        body: err instanceof Error ? err.message : t("detail.unknownError"),
        tone: "error",
      });
    },
  });
  ...
  const archiveFromInbox = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("detail.issueArchivedFromInbox"), tone: "success" });
    },
    onError: (err) => {
      pushToast({
        title: t("detail.archiveFailed"),
        body: err instanceof Error ? err.message : t("detail.unableToArchiveFromInbox"),
        tone: "error",
      });
    },
  });
  ...
  const copyIssueToClipboard = async () => {
    ...
    setCopied(true);
    pushToast({ title: t("detail.copyToClipboard"), tone: "success" });
  ...
  const treeControlPrimaryButtonLabel =
    treeControlMode === "pause"
      ? treeControlScope === "leaf"
        ? t("detail.pauseWork")
        : t("detail.pauseAndStopWork")
      : treeControlMode === "cancel"
        ? t("detail.cancelIssues", { count: previewAffectedIssueCount })
      : treeControlMode === "restore"
          ? t("detail.restoreIssues", { count: previewAffectedIssueCount })
          : treeControlScope === "leaf"
            ? t("detail.resumeWork")
    ...
    <>
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? t("detail.uploading") : (
          <>
            <span className="hidden sm:inline">{t("detail.uploadAttachment")}</span>
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-medium">
                  {childIssues.length === 0 ? t("detail.pausedByBoard") : t("detail.subtreePauseActive")}
                </span>
                <span className="text-xs text-amber-900/80 dark:text-amber-100/80">
                  {childIssues.length === 0
                    ? t("detail.issueHeldDesc")
                    : t("detail.subtreeHeldDesc")}
                </span>
              </div>
              <div className="text-xs text-amber-900/80 dark:text-amber-100/80">
                {childIssues.length === 0
                  ? t("detail.oneIssueHeld")
                  : t("detail.descendantsHeld", { count: heldDescendantCount })}
              ...
              {canShowSubtreeControls || canResumeLeafWork ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {childIssues.length === 0 ? t("detail.resumeWork") : t("detail.resumeSubtree")}
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    {t("detail.viewAffected", { count: childIssues.length === 0 ? 1 : heldDescendantCount })}
                  </Button>
        ...
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {issue.originKind === "issue_productivity_review" ? (
            <span
              ...
              title={t("detail.productivityReview")}
            >
              <Eye className="h-3 w-3" />
              {t("detail.productivityReview")}
            </span>
          ) : null}
          ...
          <div className="hidden md:flex items-center md:ml-auto shrink-0">
            {canArchiveFromInbox && (
              <Button
                ...
                disabled={archivePending}
                title={t("detail.archiveFromInbox")}
                aria-label={t("detail.archiveFromInbox")}
              >
            ...
            <Popover open={moreOpen} onOpenChange={setMoreOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  size="icon-xs"
                  className="shrink-0"
                  aria-label={t("detail.moreIssueActions")}
                  title={t("detail.moreIssueActions")}
                  onKeyDown={(event) => {
      ...
      {showRichSubIssuesSection ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-sm font-medium text-muted-foreground">{t("detail.subIssues")}</h3>
          </div>
          <IssuesList
            issues={childIssues}
      ...
      <Dialog open={treeControlOpen} onOpenChange={setTreeControlOpen}>
        <DialogContent className="flex max-h-[calc(100dvh-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-[560px]">
          <DialogHeader className="border-b border-border/60 px-6 pb-4 pr-12 pt-6">
            <DialogTitle>{treeControlLabel(t, treeControlMode, treeControlScope)}</DialogTitle>
            <DialogDescription>
              {treeControlHelpText(t, treeControlMode, treeControlScope)}
            </DialogDescription>
          </DialogHeader>
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
            {treeControlMode === "cancel" ? (
              <div className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
                {t("detail.cancelWarning")}
              </div>
            ) : null}
            ...
            {(treeControlMode === "resume" || treeControlMode === "restore") ? (
              <div className="space-y-2">
                <label className="flex items-start gap-2 text-sm">
                  ...
                  />
                  <span>
                    <span className="block font-medium">{t("detail.wakeAffectedAgents", { count: previewAffectedAgentCount })}</span>
                    <span className="text-xs text-muted-foreground">
                      {previewAffectedAgentCount === 0
                        ? t("detail.noEligibleAgents")
                        : t("detail.wakeAgentsDesc")}
                    </span>
                  </span>
                </label>
            ...
            {treeControlMode === "cancel" ? (
              <label className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-2 text-sm">
                ...
                />
                <span>{t("detail.cancelConfirm", { count: previewAffectedIssueCount })}</span>
              </label>
            ) : null}
            ...
            <div className="space-y-2">
              {treeControlPreviewLoading ? (
                ...
                <div className="space-y-2">
                  <p className="text-xs text-destructive">{treeControlPreviewErrorCopy(t, treeControlPreviewError)}</p>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                        ...
                        <div key={candidate.id} style={candidate.depth > 0 ? { paddingLeft: `${Math.min(candidate.depth, 6) * 14}px` } : undefined}>
                          ...
                          >
                            ...
                            {candidate.skipped && candidate.skipReason === "terminal_status" ? (
                              <span className="shrink-0 text-xs text-muted-foreground">{t("detail.complete")}</span>
                            ) : null}
                          </Link>
                        </div>
                      ))}
                ...
                <p className="text-xs text-muted-foreground">{t("detail.previewUnavailable")}</p>
              )}
            </div>
          </div>
